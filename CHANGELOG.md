...
### Fixed (2026-09-05)
- **Modelo 06 a DDL real:** `device_config` un solo `version`, `status` en `user`, `is_active` en `role`/`evidence`/`audit_login`, `event` transaccional (no append-only), `evidence` 1:1 con `checksum_sha256` (DDL lo exige, v1 `size_bytes` + ETag obsoleto), Resumen 33 tablas, orden real de migración, `.mmd` sin `file_url` y con `device_id`, `v_metrics_daily` vía `device_assignment`, `is_active`/FK matizados.
