...
| Tema | Regla |
|------|-------|
| Repositorios | Uno por componente: `somnguard-docs`, `somnguard-api`,`somnguard-app`, `somnguard-portal`, `somnguard-device` |
...
- Los archivos `.env.*` reales se ignoran via `.gitignore`; solo se versionan `.env.example`.
...
- La evidencia multimedia del sistema no se expone públicamente (ver `05-architecture/architecture-document.md`).
...
4. **Registrar el incidente**: abrir incidencia de seguridad y documentar en `15-project-control/risks.md`.
...
| Líder técnico | Aprueba PRs hacia `main` (ver `10-devops/ci-cd-strategy.md`) |
