...
| RF-PAR-04 | Umbrales por defecto + override por device_config (JSONB) | HU-API-005 | Parameterization | RN-PAR-04 | — | — | Integración: merge defaults+override, device pulla config |
...
| RF-DEV-03 | Config remota device_config (JSONB: umbrales, sound, volumen, sync) | HU-API-005 | Device Management | RN-DEV-04 | NFR-03 | — | Integración: PATCH/GET config, merge defaults, historial |
...
| RF-EDGE-11 | Aplicación device_config recibida (umbrales, sound_pattern, volumen, sync) | HU-DEVICE-002, HU-DEVICE-004, HU-API-005 | Device Edge | RN-EDGE-11 | NFR-03 | — | Integración: config pull tras sync, merge defaults, aplica en caliente |
