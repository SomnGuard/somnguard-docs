...
| TD-004 | **Plantilla de módulo sin instanciar**: `09-modules/modules/_template/` está lista pero ningún módulo tiene aún su documentación por módulo. | Medio | P2 | 09-modules | Resuelto (2026-09-01: `modules/security/` con README, data-model, events, decisions, features-analysis; runbook pendiente) |
| TD-005 | **Catálogos sin semillas definidas**: valores iniciales de `parameterization` fijados en `data-dictionary.md` y seeds `001..008` (categorías, severidades, sonidos, tipos, estados). | Medio | P2 | parameterization | Resuelto |
| TD-006 | **Estados de negocio sin decisión**: si usar catálogo parametrizable genérico o `VARCHAR + CHECK` para estados de dispositivo/evento (ver [modeling-conventions](../06-data-architecture/modeling-conventions.md) y Q-013 en [open-questions](./open-questions.md)). | Medio | P2 | telemetry-service, device-management | Resuelto (ADR-009 aceptada 2026-08-22: `status_category` + `status`) |
