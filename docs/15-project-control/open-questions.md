...
| QR-008 | ¿Qué formato tiene el payload de sincronización offline (JSON + multimedia)? | Lote `POST /telemetry/events` solo metadata JSON `{"events":[]}` + `POST /telemetry/events/{id}/evidence` multipart (base64 descartado) | [api-design.md](../07-api-design/api-design.md), [ADR-005](../05-architecture/decisions/records/ADR-005-offline-first-device.md) | 2026-09-04 |
| QR-013 | ¿Los estados de negocio usan catálogo parametrizable genérico o `VARCHAR + CHECK`? | Catálogo parametrizable (`status_category` + `status`, FK a `parameterization`) | [ADR-009](../05-architecture/decisions/records/ADR-009-status-parametrized-audit.md) | 2026-08-22 |
