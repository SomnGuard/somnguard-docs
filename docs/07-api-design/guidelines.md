...
- **Idempotencia (dos mecanismos, no mezclados):** telemetría usa `event_id UUIDv7` único: el lote `POST /telemetry/events` **siempre responde `201 {acked_ids[], duplicate_ids[]}`** (los duplicados se reportan, no son error). `409` solo en `POST /telemetry/events/{eventId}/evidence` si ya existe evidencia. Header `Idempotency-Key` **solo** en `POST` no-telemetría (`/devices`, `/{id}/assign`, `/auth/*`, `/devices/self-register` con serial como clave natural) y en `PATCH /rotate-key` (reintentos del portal). `PATCH /devices/{id}/config` está deprecated (`410`, ADR-011) y ya no usa el header. No usar header en `/telemetry/events`. En `self-register` repetido con mismo token+serial: `200 {device_id, status}` sin reexponer key.
